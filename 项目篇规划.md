...
1. **你是谁**：你是 miniclaude 的 actor 节点，负责实现代码
2. **你可以用什么**：列出工具及使用规则
3. **你应该怎么做**：
   - 先想清楚再动手
...
```python
ACTOR_PROMPT = """You are the actor node in miniclaude's LangGraph workflow.
...
```bash
miniclaude "帮我创建一个简易的贪吃蛇游戏代码，并执行出来给我看"
```
...
**工具注册 — [registry.py:13](src/miniclaude/tools/registry.py#L13)**
...
```
帮我创建一个 Python 项目 miniclaude，用 typer 做 CLI。项目结构如下：
...
9. CLI 入口：miniclaude <task> --workspace <path>，workspace 默认自动创建
```

**Step 2: 实现 ReAct 循环**

```
在 src/miniclaude/core/agent.py 中实现一个 ReAct 循环：
...
ACTOR_PROMPT 的内容：
"""You are the actor node in miniclaude's ReAct workflow.
...
```bash
miniclaude "帮我创建一个简易的贪吃蛇游戏代码，并执行出来给我看" -w ./workspace
```
...
```python
PLANNER_PROMPT = """You are the planner node in miniclaude's LangGraph workflow.
...
**图构建 — [workflow.py:24](src/miniclaude/graph/workflow.py#L24)**
...
**状态定义 — [state.py:60](src/miniclaude/graph/state.py#L60)**
...
**Verifier 路由 — [nodes.py:412](src/miniclaude/graph/nodes.py#L412)**
...
**Verifier 节点 — [nodes.py:221](src/miniclaude/graph/nodes.py#L221)**
...
```
在 src/miniclaude/graph/state.py 中定义 LangGraph 图的共享状态：
...
```
在 src/miniclaude/graph/nodes.py 中实现三个核心节点：
...
```
在 src/miniclaude/graph/workflow.py 中构建 LangGraph 图：
...
同时在 src/miniclaude/prompts/stage2.py 中写好：
...
```bash
miniclaude "帮我实现一个 Conway's Game of Life，要求 TDD：先写测试，再写实现，最后跑 demo" --max-attempts 3
```
...
```python
PLANNER_PROMPT = """You are the planner/supervisor node in miniclaude stage 3.
...
**Planner 的 Agent 调度工具 — [nodes.py:473](src/miniclaude/graph/nodes.py#L473)**
...
**SearchAgent Handoff — [nodes.py:536](src/miniclaude/graph/nodes.py#L536)**
...
**CodeAgent Handoff — [nodes.py:558](src/miniclaude/graph/nodes.py#L558)**
...
**SearchAgent 实现 — [search_agent.py:17](src/miniclaude/agents/search_agent.py#L17)**
...
**CodeAgent 实现 — [code_agent.py:21](src/miniclaude/agents/code_agent.py#L21)**
...
```
在 src/miniclaude/agents/search_agent.py 中实现搜索专家 Agent：
...
同时在 src/miniclaude/tools/web_search_tool.py 中实现 WebSearchTool：
...
```
在 src/miniclaude/agents/code_agent.py 中实现代码专家 Agent：
...
```
改造 src/miniclaude/graph/nodes.py 中的 planner_node：
...
6. 更新 PLANNER_PROMPT（在 src/miniclaude/prompts/stage3.py）：
   """You are the planner/supervisor node in miniclaude stage 3.
...
```
在 src/miniclaude/prompts/stage3.py 中更新 VERIFIER_PROMPT：
...
miniclaude "查阅明日方舟阿米娅的资料，编写一个HTML介绍页面，至少包含2个来源链接"
```
...
**三层 Memory 构建 — [memory.py:39](src/miniclaude/graph/memory.py#L39)**
...
**Context Monitor — [nodes.py:327](src/miniclaude/graph/nodes.py#L327)**
...
**Token 估算 — [nodes.py:460](src/miniclaude/graph/nodes.py#L460)**
...
**Context Compressor — [nodes.py:356](src/miniclaude/graph/nodes.py#L356)**
...
**LLM 压缩核心 — [nodes.py:616](src/miniclaude/graph/nodes.py#L616)**
...
**Notepad 工具 — [notepad_tool.py:21](src/miniclaude/tools/notepad_tool.py#L21)**

```python
def append_notepad(state, heading, content) -> dict:
    path = state.assert_workspace_path(state.workspace / NOTEPAD_FILE)
    existing = read_text_lossy(path) if path.exists() else "# miniclaude Notepad\n"
...
```
在 src/miniclaude/graph/memory.py 中实现三层 Memory 系统：
...
```
在 src/miniclaude/graph/nodes.py 中新增 context_monitor_node：
...
```
在 src/miniclaude/graph/nodes.py 中新增 context_compressor_node：
...
CONTEXT_COMPRESSION_PROMPT（在 src/miniclaude/prompts/stage4.py）：
"""You are the context_compressor node in miniclaude stage 4.
...
```
在 src/miniclaude/graph/workflow.py 中更新图结构：
...
export MINICLAUDE_CONTEXT_TOKEN_LIMIT=50000
miniclaude "帮我搭建一个Flask后台管理系统，包含用户认证、REST API、数据库模型、前端模板"
```
...
**恢复流程：**
```bash
miniclaude --resume .miniclaude/workspaces/workspace-xxx
```
...
**命令风险分类 — [approval.py:44](src/miniclaude/core/approval.py#L44)**
...
**审批三模式分流 — [bash_tool.py:224](src/miniclaude/tools/bash_tool.py#L224)**
...
**BashTool 执行入口 — [bash_tool.py:154](src/miniclaude/tools/bash_tool.py#L154)**
...
**Checkpoint 保存 — [checkpoint.py:47](src/miniclaude/core/checkpoint.py#L47)**
...
**主循环中的 Checkpoint + Trace 集成 — [agent.py:291](src/miniclaude/core/agent.py#L291)**
...
```
在 src/miniclaude/core/approval.py 中实现命令风险分类和审批：
...
```
在 src/miniclaude/core/checkpoint.py 中实现断点保存和恢复：
...
class CheckpointManager:
    def __init__(self, runtime, task=""):
        ...
        self.root = workspace / ".miniclaude" / "checkpoints"
...
def resume_command(workspace: Path) -> str:
    """生成恢复命令字符串：miniclaude --resume <workspace>"""
...
```
在 src/miniclaude/core/trace.py 中实现执行追踪：
...
class TraceRecorder:
    def __init__(self, runtime, task=""):
        ...
        self.root = workspace / ".miniclaude" / "traces" / self.trace_id
...
```
更新 src/miniclaude/core/agent.py 中的 stream_agent_events：
...
```
在 src/miniclaude/cli/app.py 中新增参数：
...
miniclaude "帮我搭建一个Flask后台管理系统，包含用户认证" \
...
# 查看 trace
cat .miniclaude/workspaces/workspace-xxx/.miniclaude/traces/*/timeline.md
```
...
```
用户 (飞书) → 飞书 Bot API → miniclaude Backend → Agent 执行 → 结果回传飞书
```
...
$ miniclaude
 ...
 🐾 miniclaude v0.6.0 — Stage 6: TUI + Session
...
**Intent Router — [nodes.py:85](src/miniclaude/graph/nodes.py#L85)**
...
**入口工作流 — [workflow.py:49](src/miniclaude/graph/workflow.py#L49)**
...
**多轮对话 Session — [agent.py:153](src/miniclaude/core/agent.py#L153)**
...
**TUI 审批弹窗 — [tui/approval.py:34](src/miniclaude/cli/tui/approval.py#L34)**
...
```
在 src/miniclaude/graph/workflow.py 中新增入口工作流图：
...
在 src/miniclaude/graph/nodes.py 中实现两个新节点：
...
INTENT_ROUTER_PROMPT = """You are the intent router for miniclaude.
...
CHAT_RESPONDER_PROMPT = """You are miniclaude's lightweight chat node.
...
```
在 src/miniclaude/core/session.py 中实现会话管理：

SESSION_ROOT = ".miniclaude/session"
...
```
在 src/miniclaude/core/agent.py 中新增 stream_session_events：
...
```
在 src/miniclaude/cli/tui/app.py 中用 textual 实现 TUI：

class miniclaudeTuiApp(App[None]):
    """
    界面布局：
    ┌─────────────────────────────────────────────┐
    │ 🐾 miniclaude                   session: xxx │  ← Header + 状态栏
    ├─────────────────────────────────────────────┤
    │                                             │
...
审批弹窗实现（src/miniclaude/cli/tui/approval.py）：
...
```
在 src/miniclaude/cli/tui/logo.py 中实现 ASCII Art Logo：
...
# Trace 测试：查看 .miniclaude/traces/*/timeline.md
```
...
| 文件 | 核心函数 | 阶段 | 一句话 |
|------|---------|------|--------|
| [registry.py:13](src/miniclaude/tools/registry.py#L13) | `build_tools` / `build_read_only_tools` | 1-6 | 工具注册：全量 vs 只读 |
| [workflow.py:24](src/miniclaude/graph/workflow.py#L24) | `build_complex_workflow` | 2-6 | 图结构定义：节点 + 边 + 条件路由 |
| [workflow.py:49](src/miniclaude/graph/workflow.py#L49) | `build_entry_workflow` | 6 | 入口图：intent_router → chat/workflow |
| [state.py:60](src/miniclaude/graph/state.py#L60) | `MokioGraphState` | 2-6 | 图共享状态，`messages` 用 `add_messages` |
| [nodes.py:85](src/miniclaude/graph/nodes.py#L85) | `intent_router_node` | 6 | LLM 判断意图：chat 还是 workflow |
| [nodes.py:152](src/miniclaude/graph/nodes.py#L152) | `planner_node` | 2-6 | Supervisor：调用 TodoWrite/Search/Code 工具 |
| [nodes.py:221](src/miniclaude/graph/nodes.py#L221) | `verifier_node` | 2-6 | 只读工具验证 + JSON 判断 pass/fail |
| [nodes.py:412](src/miniclaude/graph/nodes.py#L412) | `verifier_route` | 2-6 | passed→final / 达上限→final / 否则→planner |
| [nodes.py:473](src/miniclaude/graph/nodes.py#L473) | `_build_planner_tools` | 3 | Planner 的 3 个调度工具 |
| [nodes.py:536](src/miniclaude/graph/nodes.py#L536) | `_call_search_agent_tool` | 3 | Handoff: planner→searchAgent |
| [nodes.py:558](src/miniclaude/graph/nodes.py#L558) | `_call_code_agent_tool` | 3 | Handoff: planner→codeAgent |
| [search_agent.py:17](src/miniclaude/agents/search_agent.py#L17) | `run_search_agent` | 3 | 搜索专家：只绑 WebSearchTool |
| [code_agent.py:21](src/miniclaude/agents/code_agent.py#L21) | `run_code_agent` | 3 | 编码专家：全量工具 + TodoUpdate + Memory |
| [memory.py:39](src/miniclaude/graph/memory.py#L39) | `build_layered_memory` | 4 | 三层 Memory：Rules + Working + History |
| [nodes.py:327](src/miniclaude/graph/nodes.py#L327) | `context_monitor_node` | 4 | 估算 token 数，决定是否压缩 |
| [nodes.py:356](src/miniclaude/graph/nodes.py#L356) | `context_compressor_node` | 4 | LLM 摘要 + RemoveMessage 清空历史 |
| [nodes.py:616](src/miniclaude/graph/nodes.py#L616) | `_compress_context_with_model` | 4 | 压缩核心：给 LLM 全量消息，返回摘要 |
| [notepad_tool.py:21](src/miniclaude/tools/notepad_tool.py#L21) | `append_notepad` | 4 | 追加写入 NOTEPAD.md，压缩后不丢失 |
| [approval.py:44](src/miniclaude/core/approval.py#L44) | `classify_command_risk` | 5 | 正则匹配高风险命令 |
| [bash_tool.py:224](src/miniclaude/tools/bash_tool.py#L224) | `_resolve_approval` | 5 | 三模式分流：auto/deny/inline |
| [bash_tool.py:154](src/miniclaude/tools/bash_tool.py#L154) | `run_bash` | 5 | 完整流程：校验→危险拦截→审批→执行 |
| [checkpoint.py:47](src/miniclaude/core/checkpoint.py#L47) | `CheckpointManager.save` | 5 | light/strict/off 三级检查点 |
| [trace.py:29](src/miniclaude/core/trace.py#L29) | `TraceRecorder` | 5 | 逐事件记录 + 统计 + timeline.md |
| [agent.py:153](src/miniclaude/core/agent.py#L153) | `stream_session_events` | 6 | 多轮对话：session→router→workflow→session |
| [session.py:100](src/miniclaude/core/session.py#L100) | `build_session_context` | 6 | 从 session.json 构建上下文字符串 |
| [tui/approval.py:34](src/miniclaude/cli/tui/approval.py#L34) | `ApprovalGate` | 6 | 线程同步：等待人类审批决策 |
...
4. **实机演示**：运行 miniclaude，展示效果（3-5 分钟）
...
> 🐾 miniclaude — 从一行代码到一个 Agent，六步走完。
